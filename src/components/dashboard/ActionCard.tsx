...
function ActionCard({ 
  ...
  return (
    <div 
      className={`relative bg-white rounded-2xl shadow-lg p-8 hover:shadow-2xl transition-all duration-300 transform hover:scale-105 ${animationClass} border border-gray-100 hover:border-gray-200 group cursor-pointer ${className}`}
      style={{ animationDelay }}
    >
      <div className="flex items-center mb-6">
        <div className={`w-12 h-12 ${iconBgColor} rounded-xl flex items-center justify-center mr-4 transition-all duration-300 group-hover:scale-105 group-hover:rotate-2`}>
...
function QuickActionButton({ label, icon, color, onClick, href }: QuickActionButtonProps) {
  ...
  return (
    <button 
      onClick={handleClick}
      className="w-full text-left p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-all duration-300 flex items-center group/button hover:shadow-md hover:scale-[1.01] cursor-pointer"
    >
      <span className={`${color} mr-3 text-lg group-hover/button:scale-105 transition-transform duration-300`}>{icon}</span>
...
function ActivityItem({ title, time, status }: ActivityItemProps) {
  ...
  return (
    <div className={`flex items-center p-3 rounded-xl ${statusColors[status]} border-l-4 hover:shadow-md transition-all duration-300 hover:scale-[1.01] group/activity`}>
      <div className={`w-2 h-2 ${statusColors[status].split(' ')[2]} rounded-full mr-3 group-hover/activity:scale-110 transition-transform duration-300`}></div>
      <div>
